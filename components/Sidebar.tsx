...
export const Sidebar: React.FC<SidebarProps> = ({ selectedFeedId, onSelectFeed, fontSizeLevel, onCycleFontSize }) => {
  ...
  return (
    <div className="w-12 md:w-14 lg:w-64 h-full bg-gray-950 border-r border-gray-800 flex flex-col flex-shrink-0 relative z-40">
      ...
      <div className="h-16 flex items-center justify-center lg:justify-start lg:px-6 border-b border-gray-800 flex-shrink-0">
        ...
        <span className={`ml-3 font-bold text-white hidden lg:block tracking-tight ${fontSizeLevel > 0 ? 'text-2xl' : 'text-xl'}`}>Moyu</span>
      </div>
      ...
      >
        <div className={`px-4 mb-2 hidden lg:block font-semibold text-gray-500 uppercase tracking-wider ${fontSizeLevel > 1 ? 'text-sm' : 'text-xs'}`}>
            Tech Feeds
        </div>
        ...
        {FEEDS.map((feed) => (
          ...
          >
            ...
            <span className={`ml-3 font-medium hidden lg:block truncate ${fontSizeLevel > 0 ? 'text-lg' : 'text-sm'}`}>{feed.name}</span>
      ...
      <div className="border-t border-gray-800 bg-gray-950 flex-shrink-0 pb-[env(safe-area-inset-bottom)]">
        <button
          ...
          className={`w-full flex flex-col lg:flex-row items-center justify-center lg:justify-between px-0 lg:px-6 py-4 lg:py-4 transition-colors duration-200 outline-none
            ${fontSizeLevel > 0
        ...
        >
           <div className="flex items-center">
               ...
               <span className={`ml-3 font-medium hidden lg:block ${fontSizeLevel > 1 ? 'text-lg' : 'text-sm'}`}>
                  {getFontSizeLabel(fontSizeLevel)}
               </span>
           </div>
           
           {/* Level Indicators (Dots) */}
           <div className="flex space-x-1 mt-1 lg:mt-0">
               {[0, 1, 2, 3].map(level => (
                   <div 
                        key={level} 
      ...
      {hoveredFeed && (
        <div 
            className={`fixed z-50 bg-gray-800 text-white px-2 py-1.5 rounded shadow-xl border border-gray-700 lg:hidden pointer-events-none whitespace-nowrap animate-in fade-in duration-150 ${fontSizeLevel > 1 ? 'text-base' : 'text-xs'}`}
            style={{ 
                top: hoveredFeed.top, 
